...
const handleArchive = () => {
  if (confirm("Are you sure you want to archive this application? This will move it to the archive section.")) {
    ...
    <div class="flex items-center justify-between p-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div v-if="application.notes" class="mt-3">
          <div class="text-sm text-gray-600">
            <strong>Notes:</strong>
            <div class="mt-1">
              <span v-if="!isExpanded" class="break-words break-all">{{ getTruncatedText(application.notes, 80) }}</span>
              <span v-else class="break-words break-all">{{ application.notes }}</span>
              <button 
      ...
      <div class="flex-shrink-0 ml-4 flex items-center gap-3">
        ...
        <div class="flex gap-2">
          ...
          <button @click="handleArchive" class="p-2 text-gray-400 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition" title="Archive">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
